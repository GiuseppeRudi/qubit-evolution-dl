...
def feature_name_from_index(feature_index: int) -> str:
    if feature_index < 10:
        return f"m{feature_index + 1}"
    return f"c{feature_index - 9}"
...
def generate_plot_for_feature(
    ...
    plt.plot(pred_time_axis, out_a, label=label_a, linestyle="--")

    if out_b is not None:
        plt.plot(pred_time_axis, out_b, label=label_b, linestyle="--")

    plt.axvline(x=input_len - 1, linestyle=":", label="Fine Input")

    fname = feature_name_from_index(feature_index)
    ...
    filename = f"s{sample_index}_f{feature_index+1}_{fname}.png"
